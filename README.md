# 📊 Desafio de BI - Análise de Vendas Mundial

Este projeto foi desenvolvido como parte do desafio da **DIO (Digital Innovation One)**. 
Devido a restrições de conta corporativa no Power BI, utilizei **Python** e **Streamlit** para criar uma aplicação web interativa que replica os visuais solicitados.

## 🚀 Acesse o Dashboard Online
[CLIQUE AQUI PARA VISUALIZAR O DASHBOARD INTERATIVO](https://desafio-power-bi-dio.streamlit.app/)

## 🛠️ Tecnologias Utilizadas
- **Python** (Processamento de dados)
- **Streamlit** (Interface Web)
- **Plotly Express** (Gráficos Dinâmicos e Mapas)
- **Pandas** (Tratamento de bases)

## 📈 Funcionalidades
- Mapa de Vendas Mundiais
- Mapa de Lucro (Profit) por País
- Gráfico de Pizza de Lucro por Segmento
- Filtros interativos na barra lateral
